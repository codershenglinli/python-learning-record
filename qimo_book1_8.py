...
def screen_shot():
    ...
    while interrupt_flag:
        ...
        if start is not None and end is not None:
            left = int(1390)  # 考虑场地灯光，默认抢1~8号场，1100 场地坐标不需要动 考虑剑走偏锋，选择更窄的区域，避开初始时间的场 比如10号
            top = int((1105 + (start - 18) * 70))  # 截图范围的起始时间（不需要改）
            width = int(1520-left)  # 取的右边线，即12号场的位置
            height = int((end - start) * 70)  # 截图的高度由输入的时间决定
        else:
            left, top, width, height = 733, 737, 280, 200
        ...
        if np.any(mask):
            ...
            time.sleep(0.1)  # 关键在这个延迟，如何卡点选中该色块 0.2可以，但时间太长
            ...
            interrupt_flag = False
            break
        else:  # 有时候是卡住，有时候是延迟1秒才刷新。难绷啊。。。 还有重复点击的问题
            # 更新：似乎没有重复点击的问题，但是在气膜界面会出现卡顿的现象
            # print("扫描ing")
            now = datetime.now()
...
def action():
    while True:
        ...
        while interrupt_flag and start_flag:
            ...
            time.sleep(0.02)  # 513为使用乒乓球界面进行测试，486为气膜界面
