...
def screen_shot():
    global screen_x, screen_y, interrupt_flag
    while interrupt_flag:
        # 3. 根据时间选择截图位置
        if start is not None and end is not None:
            left = int(550)  # 考虑场地灯光，默认抢1~8号场，1100 场地坐标不需要动
            top = int((670 + (start - 18) * 70))  # 截图范围的起始时间（不需要改）
            width = int(650 - 550)  # 取的右边线，即12号场的位置
            height = int((end - start) * 70)  # 截图的高度由输入的时间决定
        else:
            left, top, width, height = 550, 740, 100, 460
        ...
        if np.any(mask):
            ...
            # 执行额外的点击操作
            pyautogui.click(2000, 1220)  # 点击立即下单按钮
            time.sleep(0.2)
...
def action():
    while True:
        ...
        while interrupt_flag and start_flag:
            ...
            time.sleep(0.1)
            # 2. 滑轮下移
            mouse_drag(2540, 155, 2542, 486)
            time.sleep(0.1)  # 513为使用乒乓球界面进行测试，486为气膜界面
